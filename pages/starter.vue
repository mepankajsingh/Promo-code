<template>
  <div class="max-w-4xl mx-auto px-4 py-8">
    <h1 class="text-3xl font-bold mb-6">{{ $t('starter.title') }}</h1>
    
    <div class="bg-white rounded-lg shadow-md p-6 mb-8">
      <h2 class="text-xl font-semibold mb-4">{{ $t('starter.claimTitle') }}</h2>
      ...
      <div v-else-if="claimed" class="bg-green-50 border border-green-200 rounded-md p-4 mb-6">
        <div class="flex">
          ...
          <div class="ml-3">
            <h3 class="text-sm font-medium text-green-800">{{ $t('starter.success') }}</h3>
            <div class="mt-2 text-sm text-green-700">
              <p>{{ $t('starter.codeInfo') }}</p>
      ...
      <div v-else-if="error" class="bg-red-50 border border-red-200 rounded-md p-4 mb-6">
        <div class="flex">
          ...
          <div class="ml-3">
            <h3 class="text-sm font-medium text-red-800">{{ $t('starter.error') }}</h3>
      ...
      <div v-else>
        <p class="mb-4">{{ $t('starter.description') }}</p>
        
        <form @submit.prevent="claimPromoCode" class="space-y-4">
          <div>
            <label for="email" class="block text-sm font-medium text-gray-700 mb-1">{{ $t('starter.emailLabel') }}</label>
            <input 
              id="email"
              v-model="email" 
              type="email" 
          ...
          <div>
            <label for="region" class="block text-sm font-medium text-gray-700 mb-1">{{ $t('starter.regionLabel') }}</label>
            <select 
              id="region"
              v-model="region" 
              required
